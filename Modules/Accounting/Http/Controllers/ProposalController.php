...
use Modules\Accounting\Exports\ProposalExport;
...
//use Modules\Accounting\Models\Milestone;
//use Modules\Accounting\Models\Products;
...
//use Modules\Accounting\Models\Task;
use App\Models\User;
...
class ProposalController extends AppBaseController
{
    ...
    public function index(Request $request)
    {
        if(Auth::user()->can('manage proposal'))
        {
            ...
            //$customer = User::where('created_by', '=', Auth::user()->creatorId())->get()->pluck('first_name', 'id');
            $customer = User::get()->map(function ($user) {
                return [
                    'id' => $user->id,
                    'full_name' => $user->first_name . ' ' . $user->last_name,
                ];
            })->pluck('full_name', 'id')->prepend('All', '');
    ...
    public function create($customerId)
    {
        if(Auth::user()->can('create proposal'))
        {
           ...
           // $customers       = User::where('created_by', \Auth::user()->creatorId())->get()->pluck('name', 'id');
           $customers = User::get()->map(function ($user) {
            ...
            return view('accounting::proposal.create', compact('customers', 'proposal_number', 'product_services', 'category', 'customFields', 'customerId'));
        }
        else
        {
    ...
    public function customer(Request $request)
    {
        ...
        return view('accounting::proposal.customer_detail', compact('customer'));
    }
...
//                $proposalProduct->discount    = isset($products[$i]['discount']) ? $products[$i]['discount'] : 0;
    ...
    public function edit($ids)
    {
        ...
        if(Auth::user()->can('edit proposal'))
        {
            ...
            $proposal_number = Auth::user()->proposalNumberFormat($proposal->proposal_id);
            //$customers       = User::where('created_by', Auth::user()->creatorId())->get()->pluck('first_name', 'id');
            $customers = User::get()->map(function ($user) {
                return [
                    'id' => $user->id,
                    'full_name' => $user->first_name . ' ' . $user->last_name,
...
//                    $proposalProduct->discount    = isset($products[$i]['discount']) ? $products[$i]['discount'] : 0;
    ...
    public function customerProposal(Request $request)
    {
        if(Auth::user()->can('manage customer proposal'))
        {
            ...
            return view('accounting::proposal.index', compact('proposals', 'status'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission Denied.'));
    ...
    public function customerProposalShow($ids)
    {
        if(Auth::user()->can('show proposal'))
        {
            ...
            if($proposal->created_by == Auth::user()->creatorId())
            {
                ...
                return view('accounting::proposal.view', compact('proposal', 'customer', 'iteams'));
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
...
//                dd($proposalArr);
    ...
    public function proposal($proposal_id)
    {
        ...
        if($proposal)
        {
            ...
            return view('accounting::proposal.templates.' . $settings['proposal_template'], compact('proposal', 'color', 'settings', 'customer', 'img', 'font_color', 'customFields'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function invoiceLink($proposalID)
    {
        ...
        return view('accounting::proposal.customer_proposal',compact('proposal','customer','iteams','customFields','status','user'));
    }
