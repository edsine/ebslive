...
use Modules\Accounting\Mail\TestMail;
use Modules\Accounting\Models\EmailTemplate;
use Modules\Accounting\Models\ExperienceCertificate;
use Modules\Accounting\Models\GenerateOfferLetter;
use Modules\Accounting\Models\IpRestrict;
use Modules\Accounting\Models\JoiningLetter;
use Modules\Accounting\Models\Language;
use Modules\Accounting\Models\NOC;
use App\Models\User;
...
class SystemController extends AppBaseController
{
    public function index()
    {
        if(Auth::user()->can('manage company settings'))
        {
            ...
            return view('accounting::accounting::settings.index', compact('settings','admin_payment_setting'));
        }
        else
        {
...
//                }
    ...
    public function companyIndex(Request $request )
    {
        if(Auth::user()->can('manage company settings'))
        {
            ...
            return view('accounting::settings.company', compact('settings','company_payment_setting',
...
//        $user = Auth::user();
        ...
        return view('accounting::settings.test_mail', compact('data'));
    }
    ...
    public function posPrintIndex()
    {
        if(Auth::user()->can('manage print settings'))
        {
            ...
            return view('accounting::settings.pos', compact('settings'));
        }
        else
        {
            return redirect()->back()->with('error', 'Permission denied.');
