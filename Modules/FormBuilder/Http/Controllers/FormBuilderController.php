...
use Modules\FormBuilder\Models\FormFieldResponse;
        ...
        if(!empty($code))
        {
            ...
            if(!empty($form))
            {
                if($form->is_active == 1)
                {
                    ...
                    return view('formbuilder::form_builder.form_view', compact('objFields', 'code', 'form'));
                }
                else
                {
                    return view('formbuilder::form_builder.form_view', compact('code', 'form'));
                }
            }
            else
            {
        ...
        { */
            ...
            if($form->created_by == $usr->id)
            {
               ...
               // $pipelines = Pipeline::where('created_by', '=', $usr->id)->get()->pluck('name', 'id');
                ...
                return view('formbuilder::form_builder.form_field', compact('form', 'types', 'formField', 'users'));
            }
            else
            {
                return redirect()->back()->with('error', __('Permission Denied.'));
