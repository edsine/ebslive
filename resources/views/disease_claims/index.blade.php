...
@include('layouts.messages')
    ...
    <div class="nk-block-head nk-block-head-sm">
        <div class="nk-block-between">
            ...
            <div class="row " >
                <div class="col-sm-12 " style="text-align: end"><a href="{{ route('employer.diseaseclaims.create', [$employer->id]) }}" class="btn btn-primary"><em
                    ...
                    <tbody>
                        @foreach ($claims as $claim)
                            <tr>
                                ...
                                {{-- <td>
                                    @if (!$claim->request && $claim->status == 0)
                                        ...
                                        document.getElementById('process-claim-form').submit();">Initiate
                                            Processing</a>
                                        <form id="process-claim-form" action="/claim/disease/{{ $claim->id }}"
                                            method="POST">
                                            @csrf
                                            @method('PUT')
                                ...
                                <td>
                                    <a href="{{ route('employer.disease.claims.show', ['id' => $claim->id]) }}" class="text-dark"><i class="fa fa-eye"></i> View Details</a>
