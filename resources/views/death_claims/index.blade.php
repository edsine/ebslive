    ...
    <div class="nk-block-head nk-block-head-sm" >
        <div class="nk-block-between">
            <div class="nk-block-head-content">
                <h3 class="nk-block-title page-title">Death Claim for {{ $employer->contact_firstname .' '.$employer->contact_surname ?? '' }} {{ '['.$employer->company_name.']' ?? '' }}</h3>

            </div><!-- .nk-block-head-content -->
             <div class="row " >
                <div class="col-sm-12 " style="text-align: end">
                    <a href="{{ route('employer.deathclaims.create', [$employer->id]) }}" class="btn btn-primary"><em
                ...
                <table class="datatable-init-export nowrap table" data-export-title="Export">
                    ...
                    <tbody>
                        @foreach ($claims as $claim)
                            <tr>
                                <td>{{ $claim->last_name }} {{ $claim->first_name }}
                                    {{ $claim->middle_name }}</td>
                                {{-- <td>{{ $claim->gender }}</td> --}}
                                ...
                                <td>
                                    <a href="{{ route('employer.death.claims.show', ['id' => $claim->id]) }}" class="text-dark"><i class="fa fa-eye"></i> View Details</a>
