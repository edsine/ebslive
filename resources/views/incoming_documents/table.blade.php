        ...
        <table class="table align-middle gs-0 gy-4" id="order-listing">
            ...
            <tbody>
                ...
                @foreach ($documents as $index => $document)
                    ...
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ $document->title ? $document->title : 'NILL' }}</td>
                       ...
                       {{--  <td><a class="document-link" target="_blank" href="{{ asset($document->document_url) }}" onClick="saveData('{{ $document->document_url }}')">{{ substr($document->document_url, 10) }} </a>
                        ...
                        <td><a class="document-link" target="_blank" href="{{ env('DOCUMENT_URL').$document->document_url }}" onClick="saveData('{{ $document->document_url }}')">{{ substr($document->document_url, 10) }} </a>
                        </td>
...
/* $('#historyModal1').on('hidden.bs.modal', function (e) {
    // Remove the backdrop
    $('.modal-backdrop').remove();
});
// Initialize modal with backdrop set to false
$('#historyModal').modal({
    backdrop: false
}); */
